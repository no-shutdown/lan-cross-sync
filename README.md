...
- 回环抑制只记住“本机剪贴板当前的内容”，不记录历史。同一段内容隔一会儿再复制一次，仍然会同步；对方把先前同步过的内容再发回来，只要本机剪贴板已经变过，也会照常写入。
- 图片写入系统剪贴板后再读回来会得到重新编码的 PNG 字节，哈希对不上，因此写入远端图片后的 2 秒内，第一次“图片变化”被判定为自己写入的回声，不再发回对端。文本往返是逐字节一致的，不受这个时间窗影响。
- 一次剪贴板都没有发出去时（例如对端刚好断开），本机不会把这份内容标记为已处理，下一次轮询或重新复制会重试，而不是静默丢弃。
- 剪贴板读取失败不会终止监听：轮询循环和 Windows 监听线程都会继续运行，Windows 监听线程结束后会在 1 秒后自动重建。
- 单条文本剪贴板上限为 1 MiB。超过上限的文本在本地就被丢弃，不会因为超出 8 MiB 传输帧上限而拖垮整条连接。
- Windows 收到网页或其他应用的“复制”按钮通知后，会在阻塞线程中最多重试 5 次读取剪贴板，每次间隔 20 ms，以覆盖应用异步写入剪贴板的短暂窗口。
- 图片剪贴板会先编码为 PNG，再以 64 KiB 分片传输；单张图片的编码后上限为 64 MiB，本地解码后的 RGBA 数据上限为 128 MiB。传输帧上限仍为 8 MiB。
- 新的图片分片协议使用业务协议版本 3，两台设备必须运行支持协议版本 3 的构建才能同步大图片。
- 文件接收先写入临时目录；分片校验或写入失败会通知发送端取消并清理 `.part` 文件，多文件落盘中途失败时会回滚已经移动的文件。
