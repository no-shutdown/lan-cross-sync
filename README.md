...
- **可被发现**：控制本机是否向子网广播自己的信息（发送侧）。关闭后默认不再广播；但只要有已配对设备当前不是"已连接"状态、或本机正在生成配对码等待被扫描，会自动临时恢复广播，直到条件消失。
- **搜索设备**：控制是否把收到的、尚未配对的设备加入"已发现设备"列表（接收侧）。关闭后不再显示新设备，并会立即清空当前列表；已配对设备的重连和信息同步不受影响。

已配对且已连接的设备之间，不管"可被发现"开关状态如何，每 3 秒都会互相单播一次心跳包，用于同步对方最新的设备信息（比如改名）并维持连接存活。一旦某个已配对设备不是"已连接"状态（掉线、换网络、应用刚启动等），本机会自动临时恢复广播帮助双方重新发现对方——设备当前 IP 不落盘持久化，所以每次应用重启都会经过这条路径重新建立连接，不需要额外的缓存机制。
