...
- **可被发现**：控制本机是否向子网广播自己的信息（发送侧）。关闭后就真的不再广播，没有例外——生成配对码依赖"能被对方扫到"，所以要求"可被发现"必须先开启，未开启时"生成配对码"按钮直接禁用；配对过程中如果把"可被发现"关掉，正在等待的配对码会被自动取消。关闭时不会因为"有已配对设备掉线"而自动恢复广播：重连只需要**至少一方**在广播，只要对方的"可被发现"还开着，本机接收后直接反向拨号即可重连，不需要本机也跟着广播。如果两台已配对设备同时关闭"可被发现"、且这期间恰好有一方 IP 变了，会连不上，需要任意一方手动临时打开开关。
- **搜索设备**：控制是否把收到的、尚未配对的设备加入"已发现设备"列表（接收侧）。关闭后不再显示新设备，并会立即清空当前列表；已配对设备的重连和信息同步不受影响。

已配对且已连接的设备之间，不管"可被发现"开关状态如何，每 3 秒都会互相单播一次心跳包，用于同步对方最新的设备信息（比如改名）并维持连接存活；设备当前 IP 不落盘持久化，重连完全依赖收到对方广播/心跳来重新获知对方地址。
