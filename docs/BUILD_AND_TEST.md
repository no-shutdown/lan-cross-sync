...
两台设备连接同一个局域网，并确保 Windows 防火墙允许应用在专用网络通信。macOS 首次运行时允许应用访问“本地网络”；如果此前选择过拒绝，需要在“系统设置 -> 隐私与安全性 -> 本地网络”中重新开启。分别启动应用后按以下顺序验收：

1. 两台设备都出现在“发现的设备”列表。
2. 设备 A 生成配对码，设备 B 输入 6 位配对码并完成配对。
3. 两边的设备都进入已配对列表；重启其中一台后，连接状态可以恢复。
4. 在网页或其他应用中点击“复制”按钮复制文本和图片，确认另一台可以粘贴；再验证一张超过 8 MiB 原始传输帧大小的高分辨率截图可以同步；关闭某个设备的接收剪贴板开关后，确认该设备不再被写入。
...
8. 退出其中一台未配对设备，等待约 10 秒，另一台的“发现的设备”列表应自动移除该设备；重新启动后应再次出现。
